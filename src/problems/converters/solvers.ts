...
export const solveKilometerToMeter = (inputs: { [key: string]: string }): string => {
...
export const solveCelsiusToFahrenheit = (inputs: { [key: string]: string }): string => {
    const celsius = parseNumber(inputs.celsius, 'Celsius');
    const fahrenheit = (celsius * 9/5) + 32;
    return `${celsius}°C is equal to ${fahrenheit.toFixed(2)}°F.`;
};

export const solveToUppercase = (inputs: { [key: string]: string }): string => `Uppercase: ${inputs.str.toUpperCase()}`;

export const solveToLowercase = (inputs: { [key: string]: string }): string => `Lowercase: ${inputs.str.toLowerCase()}`;

export const solveBinaryToDecimal = (inputs: { [key: string]: string }): string => {
    if (!/^[01]+$/.test(inputs.binary)) throw new Error("Invalid binary string.");
    return `Decimal: ${parseInt(inputs.binary, 2)}`;
};

export const solveDecimalToBinary = (inputs: { [key: string]: string }): string => `Binary: ${parseInteger(inputs.decimal, 'Decimal').toString(2)}`;

export const solveFahrenheitToCelsius = (inputs: { [key: string]: string }): string => {
    const f = parseNumber(inputs.fahrenheit, 'Fahrenheit');
    return `${f}°F is ${((f - 32) * 5/9).toFixed(2)}°C.`;
};

export const solveBaseConverter = (inputs: { [key: string]: string }): string => {
...
export const solveStringToAscii = (inputs: { [key: string]: string }): string => {
    return `ASCII: [${inputs.str.split('').map(c=>c.charCodeAt(0)).join(', ')}]`;
};

export const solveAsciiToString = (inputs: { [key: string]: string }): string => {
    const codes = parseNumberArray(inputs.ascii);
    return `String: "${String.fromCharCode(...codes)}"`;
};

export const solveKelvinToCelsius = (inputs: { [key: string]: string }): string => `${inputs.kelvin}K is ${(parseNumber(inputs.kelvin, 'Kelvin') - 273.15).toFixed(2)}°C.`;

export const solveHexToDecimal = (inputs: { [key: string]: string }): string => {
    if(!/^[0-9A-F]+$/i.test(inputs.hex)) throw new Error("Invalid hexadecimal string.");
    return `Decimal: ${parseInt(inputs.hex, 16)}`;
};

export const solveDecimalToHex = (inputs: { [key: string]: string }): string => `Hex: ${parseInteger(inputs.dec, 'Decimal').toString(16).toUpperCase()}`;
...
export const solveRomanToNum = (inputs: { [key: string]: string }): string => {
    ...
    for (let i = 0; i < roman.length; i++) {
        ...
        if (!current) throw new Error(`Invalid Roman numeral character: ${roman[i]}`);
