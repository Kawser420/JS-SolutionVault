...
import { SparklesIcon, CodeIcon, CopyIcon, CheckCircleIcon, XCircleIcon, ChevronDownIcon } from './icons/Icons';

interface ProblemCardProps {
  problem: Problem;
}

const InputField: React.FC<{ input: ProblemInput, value: string, onChange: (id: string, value: string) => void }> = ({ input, value, onChange }) => {
  return (
    <div key={input.id}>
      <label htmlFor={input.id} className="block text-sm font-medium text-[--text-color]/80 mb-1">
        {input.label}
      </label>
      <input
        type={input.type}
...
const ProblemCardComponent: React.FC<ProblemCardProps> = ({ problem }) => {
  ...
  const handleExplain = async () => {
    ...
    try {
      ...
      const aiExplanation = await getExplanation(problem.title, problem.solverName || 'anonymous', solverCode);
  ...
  const solverCode = useMemo(() => {
    ...
    if (bodyMatch && bodyMatch[1]) {
        return bodyMatch[1].trim().replace(/(\r\n|\n|\r)/gm, "\n").split('\n').map(line => line.substring(4)).join('\n');
    }
    return fnString;
  }, [problem.solver]);
  ...
  return (
    <div className="bg-[--card-bg] border border-[--border-color] rounded-lg shadow-sm flex flex-col transition-all duration-300 animate-[fade-in_0.5s_ease-out]">
      <div className="p-6">
        <h3 className="text-lg font-semibold text-[--text-color]">{problem.title}</h3>
      </div>
      ...
      <div className="border-t border-[--border-color] mt-auto">
        {(isCodeVisible || isExplanationVisible) && <div className="border-b border-[--border-color] my-0" />}

        {isCodeVisible && (
            <div className="p-4 bg-[--bg-color] relative">
                <div className="flex justify-between items-center mb-2">
                    <h4 className="text-sm font-semibold uppercase tracking-wider text-[--text-color]/70">Solution Code</h4>
                    <button onClick={handleCopyCode} className="text-[--text-color]/60 hover:text-[--primary-color] transition-colors text-xs flex items-center gap-1">
                        <CopyIcon className="w-4 h-4" /> {copySuccess || 'Copy'}
                    </button>
                </div>
                <pre className="text-xs bg-transparent p-0 rounded-md overflow-x-auto"><code className="language-js font-mono">{solverCode}</code></pre>
            </div>
        )}

        {isExplanationVisible && (
            <div className="p-4">
            <h4 className="text-sm font-semibold uppercase tracking-wider text-[--text-color]/70 mb-2">AI Explanation</h4>
        ...
        <div className="p-2 flex justify-end gap-2 text-xs">
          ...
          >
            ...
            <span>{isCodeVisible ? 'Hide' : 'Show'} Code</span>
             <ChevronDownIcon className={`w-4 h-4 transition-transform ${isCodeVisible ? 'rotate-180' : ''}`} />
          </button>
          <button
